...
class TestGetPRDiffUseCase:
    ...
    def test_cache_hit_returns_cached_result(
        self, use_case, mock_pr_diff_service, mock_cache_service, sample_pr_diff
    ):
        ...
        # Act
        result = asyncio.run(
            use_case.execute(repo_owner="owner", repo_name="repo", pr_number=123)
        )
    ...
    def test_cache_miss_fetches_and_caches(
        ...
        mock_cache_service.set.assert_called_once_with(
            "owner/repo/pr/123", "abc123", sample_pr_diff
        )

    def test_returns_none_when_commit_sha_not_found(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
    ...
    def test_returns_none_when_diff_fetch_fails(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
    ...
    def test_does_not_cache_none_result(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
        ...
        # Act
        asyncio.run(
            use_case.execute(repo_owner="owner", repo_name="repo", pr_number=99)
        )

        # Assert
        mock_cache_service.set.assert_not_called()

    def test_propagates_service_exception(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
    ...
    def test_propagates_commit_sha_exception(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
        """Test that exceptions from get_latest_commit_sha propagate."""
        # Arrange
        mock_pr_diff_service.get_latest_commit_sha.side_effect = RuntimeError(
            "Connection failed"
        )
    ...
    def test_propagates_cache_get_exception(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
    ...
    def test_cache_key_generation(
        ...
        # Act
        asyncio.run(
            use_case.execute(repo_owner="my-org", repo_name="my-repo", pr_number=42)
        )

        # Assert
        mock_cache_service.get_cache_key.assert_called_once_with(
            "my-org", "my-repo", 42
        )

    def test_commit_sha_passed_to_cache_get(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
        ...
        mock_cache_service.get.assert_called_once_with(
            "owner/repo/pr/123", "sha_xyz789"
        )

    def test_empty_string_commit_sha_returns_none(
        self, use_case, mock_pr_diff_service, mock_cache_service
    ):
