"""Domain-specific exception hierarchy for CCPRAgents.
...
class AuthenticationError(CCPRAgentsException):
...
class AuthorizationError(CCPRAgentsException):
...
class ValidationError(CCPRAgentsException):
...
class CacheError(CCPRAgentsException):
...
class ConfigurationError(CCPRAgentsException):
...
class ProcessingError(CCPRAgentsException):
...
class ResourceError(CCPRAgentsException):
...
class SecurityError(CCPRAgentsException):
...
def get_exception_details(exception: Exception) -> Dict[str, Any]:
    ...
    if isinstance(exception, CCPRAgentsException):
