...
class GetPRDiffUseCase:
    ...
    async def execute(
        ...
        result = self._pr_diff_service.get_pr_diff(repo_owner, repo_name, pr_number)
        if result:
            self._cache_service.set(cache_key, current_commit_sha, result)
        return result


class GetPRDiffUseCaseLegacy:
    """Legacy use case that depends on repository interface for backward compatibility."""
