...
This directory contains security components for input validation and sanitization to prevent common security vulnerabilities in the CCPRAgents MCP server.
...
```python
from ccpragents.infrastructure.security.input_validator import validate_github_url
...
```python
from ccpragents.infrastructure.security.input_validator import validate_repository_identifier
...
```python
from ccpragents.infrastructure.security.input_validator import sanitize_string
...
```python
from ccpragents.infrastructure.security.input_validator import validate_token
...
```python
from ccpragents.infrastructure.security.input_validator import validate_user_id
...
All validation errors raise specific exceptions from `ccpragents.domain.exceptions`:
...
import pytest
from ccpragents.infrastructure.security.input_validator import InputValidator
from ccpragents.domain.exceptions import InvalidURLError, SuspiciousOperationError
...
The `InputValidator` is integrated into `FastMCPServer` (ccpragents/application/mcp_server.py):
